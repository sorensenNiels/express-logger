...
import { IdempotencyRequest, IdempotencyResource, IdempotencyResponse } from '../models/models';
...
describe('In memory data adapter', () => {
  ...
  it('finds correct idempotency key from multiple resource insertion', async () => {
    ...
    const idempotencyResourceToFind: IdempotencyResource = idempotencyResources[index];

    // Find the resource
    const idempotencyFound = await dataAdapter.findByIdempotencyKey(idempotencyResourceToFind.idempotencyKey);
  ...
  it('updates resource with response', async () => {
    ...
    const idempotencyFoundAgain = await dataAdapter.findByIdempotencyKey(idempotencyResourceToFind.idempotencyKey);
  ...
  it('deletes resource', async () => {
    ...
    // Find the resource
    let idempotencyFound = await dataAdapter.findByIdempotencyKey(idempotencyResourceToFind.idempotencyKey);
    assert.ok(idempotencyFound);
    ...
    idempotencyFound = await dataAdapter.findByIdempotencyKey(idempotencyResourceToFind.idempotencyKey);
