...
describe('Idempotency service', () => {
  ...
  it('returns same response for same idempotency key', async () => {
    ...
    await idempotencyService.provideMiddlewareFunction(firstReq, firstRes, firstNextSpy);
    ...
    try {
      await idempotencyService.provideMiddlewareFunction(conflictReq, conflictRes, conflictNextSpy);
    ...
    await idempotencyService.provideMiddlewareFunction(secondReq, secondRes, secondNextSpy);
  ...
  it('removes resource if error reported', async () => {
    ...
    await idempotencyService.provideMiddlewareFunction(req, httpMocks.createResponse(), nextFunc);
    assert.isTrue(nextFunc.called);
    idempotencyService.reportError(req);
    await wait(1);

    try {
      await idempotencyService.provideMiddlewareFunction(req, httpMocks.createResponse(), sinon.mock());
      assert.isFalse(idempotencyService.isHit(req));
    } catch (err) {
  ...
  it('indicates misuse of the idempotency key', async () => {
    ...
    await idempotencyService.provideMiddlewareFunction(req1, httpMocks.createResponse(), sinon.spy());
    await wait(1);
    try {
      await idempotencyService.provideMiddlewareFunction(req2, httpMocks.createResponse(), sinon.spy());
  ...
  it('ignores response if not valid for persistence', async () => {
    ...
    const persistanceValidationStud = sinon.stub(responseValidator, 'isValidForPersistence').returns(false);

    await idempotencyService.provideMiddlewareFunction(req, res, sinon.mock());
    res.writeHead(200);
    res.send('something');
