...
import { IdempotencyOptions, IdempotencyRequest, IdempotencyResource, IdempotencyResponse } from '../models/models';
...
export class IdempotencyService {
  ...
  public async provideMiddlewareFunction(
    ...
    if (idempotencyKey) {
      ...
      let resource = await this._options.dataAdapter.findByIdempotencyKey(idempotencyKey);
      if (resource) {
   ...
   * @param req
   */
  private convertToIdempotencyRequest(req: express.Request): IdempotencyRequest {
    return {
      body: req.body,
      headers: req.headers,
      method: req.method,
   ...
   * @param resource
   */
  private setupHooks(res: express.Response, resource: IdempotencyResource): void {
   ...
   * @param body
   */
  private buildIdempotencyResponse(res: express.Response, statusCode: number, body: any): IdempotencyResponse {
